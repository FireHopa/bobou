...
def _build_layout_guardrail_contract(
    ...
    if landscape and source_copy_side == "right":
        slots = {
            ...
            "copy_column": (58.0, 10.0, 95.0, 73.0),
            "price": (56.0, 24.0, 94.5, 55.0),
            "footer_logos": (66.0, 76.0, 95.0, 88.0),
            "footer_cta": (65.0, 85.0, 95.0, 94.0),
        }
        hard_rules = [
            "a coluna de texto deve permanecer visualmente ancorada à direita, nunca centralizada no canvas, com o grupo ancorado à direita, mas mantendo um pequeno respiro visual antes da borda final, sem grande faixa vazia depois dele",
        ...
        slots = {
            ...
            "copy_column": (5.0, 10.0, 42.0, 73.0),
            "price": (5.5, 24.0, 44.0, 55.0),
            "footer_logos": (5.0, 76.0, 34.0, 88.0),
            "footer_cta": (5.0, 85.0, 35.0, 94.0),
        }
        hard_rules = [
            "a coluna de texto deve permanecer visualmente ancorada à esquerda, nunca centralizada no canvas, com o grupo ancorado à esquerda, mas mantendo um pequeno respiro visual antes da borda final, sem grande faixa vazia antes dele",
        ...
        slots = {
            ...
            "copy_column": (41.0, 8.5, 90.0, 66.0),
            "price": (38.0, 34.0, 67.0, 52.0),
            "footer_logos": (59.0, 66.0, 90.0, 76.0),
            "footer_cta": (48.0, 75.0, 90.0, 84.0),
        }
        hard_rules = [
            "a versão vertical deve parecer peça nativa em portrait, não crop da horizontal",
            "a coluna de copy continua ancorada no alto/direita, mas com respiro lateral e dentro da safe area x 10%–90%",
            "título, descrição, data, logos e CTA não podem encostar na borda direita; preserve uma margem visual clara, porque haverá fechamento lateral para 768x1376",
        ...
        slots = {
            ...
            "copy_column": (10.0, 8.5, 59.0, 66.0),
            "price": (33.0, 34.0, 62.0, 52.0),
            "footer_logos": (10.0, 66.0, 41.0, 76.0),
            "footer_cta": (10.0, 75.0, 52.0, 84.0),
        }
        hard_rules = [
            "a versão vertical deve parecer peça nativa em portrait, não crop da horizontal",
            "a coluna de copy continua ancorada no alto/esquerda, mas com respiro lateral e dentro da safe area x 10%–90%",
            "título, descrição, data, logos e CTA não podem encostar na borda esquerda; preserve uma margem visual clara",
        ...
        hard_rules = [
            ...
            "em portrait, mantenha todos os elementos críticos dentro da safe area lateral x 10%–90% e deixe respiro visível até a borda",
        ]
...
def _evaluate_layout_guardrails(
    ...
    for item in final_items:
        ...
        if role in {"title", "overline", "subtitle", "price", "date", "logo", "seal", "cta"}:
            if y1 < 4.5 or y2 > 95.5:
                issues.append(f"{role}_edge_risk_vertical")
            if x1 < 3.5 or x2 > 96.5:
    ...
    if group_edges:
        ...
        if source_copy_side == "right":
            if orientation == "landscape":
                ...
                if gx2 > 95.0:
                    issues.append("copy_group_too_close_outer_edge_right")
                if group_center_x < 62.0:
                    issues.append("copy_group_drifted_from_right")
            else:
                if gx2 < 84.5:
                    issues.append("copy_group_not_flush_right_portrait")
                if gx2 > 90.5:
                    issues.append("copy_group_too_close_outer_edge_right_portrait")
                if group_center_x < 49.0:
            ...
            if orientation == "landscape":
                ...
                if gx1 < 5.0:
                    issues.append("copy_group_too_close_outer_edge_left")
                if group_center_x > 38.0:
                    issues.append("copy_group_drifted_from_left")
            else:
                if gx1 > 15.5:
                    issues.append("copy_group_not_flush_left_portrait")
                if gx1 < 9.5:
                    issues.append("copy_group_too_close_outer_edge_left_portrait")
                if group_center_x > 51.0:
                    issues.append("copy_group_drifted_from_left_portrait")
        else:
            # Composição centralizada: manter respiro lateral e não deixar o bloco colado nas bordas.
            if orientation == "portrait":
                if gx1 < 8.5 or gx2 > 91.5:
                    issues.append("copy_group_too_close_outer_edge_center_portrait")
                if abs(group_center_x - 50.0) > 6.0:
                    issues.append("copy_group_not_centered_portrait")
            else:
                if gx1 < 6.0 or gx2 > 94.0:
                    issues.append("copy_group_too_close_outer_edge_center")
        ...
        if orientation == "landscape" and gy1 < 5.0:
            issues.append("copy_group_too_close_top")
        if orientation == "landscape" and gy2 > 95.5:
            issues.append("copy_group_too_close_bottom")
        if orientation == "portrait" and gy1 < 5.5:
            issues.append("copy_group_too_close_top_portrait")
        if orientation == "portrait" and gy2 > 93.5:
            issues.append("copy_group_too_close_bottom_portrait")
        ...
        if orientation == "portrait" and (gy2 - gy1) > 86.0:
    ...
    if cta_boxes:
        ...
        if orientation == "portrait":
            ...
            if y2 > 96.2:
                issues.append("cta_too_low_or_cut_risk")
            if source_copy_side == "right" and x1 < 57.0:
                issues.append("cta_not_anchored_right")
            if source_copy_side == "left" and x2 > 43.0:
                issues.append("cta_not_anchored_left")
    ...
    if orientation == "portrait" and logo_boxes:
        for box in logo_boxes:
            ...
            if x1 < 8.0 or x2 > 92.0 or y2 > 87.5:
